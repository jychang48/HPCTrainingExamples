...
contains
  ...
  subroutine init_jacobi(this,mesh)
    ...
    do j=1,mesh%n_y
      do i=1,mesh%n_x
        this%u(i,j)=0._RK ! initial guess
        this%rhs(i,j)=0._RK ! forcing function
      end do
    end do
  ...
  subroutine print_2D(array)
    ...
    integer(kind=IK) :: row, array_shape(2)
  ...
  subroutine print_results(this,mesh)
    ...
    write(stdout,'(A,F5.3,A)') 'Measured lattice updates: ',lattice_updates/this%elapsed/1.e9_RK,' LU/s'
